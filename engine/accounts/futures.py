...
from configs import SLIPPAGE, FEE_STRUCTURE, FUTURES_MAX_LEVERAGE
from configs.constants import LONG_POSITION_FUTURES, SHORT_POSITION_FUTURES
import pandas as pd
from engine.accounts.liquidation_price import calculate_liquidation_price


class FuturesAccount(Account):
    """A class representing a futures trading account.
    It inherits from the Account class and implements methods specific to futures trading.
    Parameters:
        acc_name (str): Name of the account. If None, defaults to "FuturesAccount".
        acc_type (str): Type of the account, should be "futures".
    ...
    Methods:
        open(order, ts, md, fee_calc): Opens a new order in the futures account.
        close(order_id, ts, md, fee_calc): Closes an existing order in the futures account.
        ...
        close_all_open_orders(ts, md, fee_calc): Closes all open orders in the futures account.
    Raises:
        ValueError: If the order mode is not "futures" or if the order is already closed.
    ...
    def _count_binance_funding_events(self, start_ts, end_ts):
        """
        Counts how many Binance funding fee events occurred between two timestamps.
        Funding occurs every 8 hours at 00:00, 08:00, and 16:00 UTC.
        Args:
            start_ts (pd.Timestamp): Start time in UTC (e.g., pd.Timestamp('2025-07-01 00:00:00'))
            end_ts (pd.Timestamp): End time in UTC

        Returns:
            int: Number of funding events between the two timestamps
        """
        # Round up to the next funding interval if needed
        start_hour = (start_ts.hour // 8) * 8
        first_funding_time = pd.Timestamp(
            year=start_ts.year,
            month=start_ts.month,
            day=start_ts.day,
            hour=start_hour,
            tz=start_ts.tz,  # keeps UTC if present
        )

        if first_funding_time < start_ts:
            first_funding_time += pd.Timedelta(hours=8)

        count = 0
        current_time = first_funding_time
    ...
    def _calculate_close_final_cash_value(self, px, order, ts, fee_calc):
        ...
        Returns:
            tuple: A tuple containing the closed amount in user terms, PnL, fee, funding, and refund margin.
        Raises:
            ValueError: If the order quantity is non-positive after calculating maximum open quantity.
        """
        ...
        #     md.get_price(order.asset, ts), order.side, SLIPPAGE["futures"]
        # )
        ...
        # fee = fee_calc.trade_fee("futures", self.acc_type, order.qty * px)
        # funding = fee_calc.funding_fee(order.qty * px, hours)
        # refund_margin = order.qty * order.entry_price / order.leverage
        """

        total_funding_events = self._count_binance_funding_events(
            start_ts=order.open_ts, end_ts=ts
        )
        exit_amount_notional = order.qty * px
        ...
        fee = fee_calc.trade_fee("futures", self.acc_type, exit_amount_notional)
        # TODO: add proper logic for funding fee
        funding = fee_calc.funding_fee(exit_amount_notional, total_funding_events)
        refund_margin = order.qty * order.entry_price / order.leverage

        closed_amount_user = pnl + refund_margin - fee - funding
        return closed_amount_user, pnl, fee, funding, refund_margin

    def open(self, order, ts, md, fee_calc):
        """
        Open a new order in the futures account.
        Parameters:
        ...
        if order.leverage > FUTURES_MAX_LEVERAGE:
            print(
                f"Order {order.id} has leverage greater than {FUTURES_MAX_LEVERAGE} : {order.leverage}."
            )
            return
    ...
    def close(self, order_id, ts, md, fee_calc):
        """
        Close an existing order in the futures account.
        Parameters:
        ...
        Raises:
            ...
            ValueError: If the order mode is not "futures".
        """
        ...
        closed_amount_user, pnl, fee, funding, refund_margin = (
            self._calculate_close_final_cash_value(px, order, ts, fee_calc)
        )

        self.cash += pnl + refund_margin - fee - funding
        pnl_percentage_100 = (
        ...
        order.closed_amount_user = pnl + refund_margin - fee - funding
    ...
    def update_portfolio_value(self, current_prices, ts, fee_calc):
        ...
        Returns:
            None
        Raises:
            ValueError: If the current price for an asset is not available.
        """
    ...
    def max_open_qty(self, price, leverage, side="long", is_price_slippaged=False):
        ...
        if is_price_slippaged:
            ...
            price = self._apply_slippage(price, side, SLIPPAGE["futures"])
        px = price
        ...
        fee_rate = FEE_STRUCTURE["futures"][self.acc_type]
    ...
    def close_all_open_orders(self, ts, md, fee_calc):
        """
        Close all open orders in the futures account.
    ...
    def close_all_open_orders_by_asset(self, asset, ts, md, fee_calc):
        """
        Close all open orders for a specific asset in the futures account.
        Parameters:
    ...
    def close_all_open_orders_by_asset_and_side(self, asset, side, ts, md, fee_calc):
        """
        Close all open orders for a specific asset and side in the futures account.
        Parameters:
            asset (str): The asset for which to close open orders.
            side (str): The side of the order, either "long" or "short".
            ts (pd.Timestamp): The timestamp at which the orders are closed.
    ...
    def get_open_orders_long_short_counts_by_asset(self, asset):
        """
        Get the count of open long and short orders for a specific asset in the futures account.
        Parameters:
        ...
        long_count = sum(
            ...
            if o.asset == asset and o.side == LONG_POSITION_FUTURES
        )
        short_count = sum(
            1
            for o in self.open_orders
            if o.asset == asset and o.side == SHORT_POSITION_FUTURES
        )
